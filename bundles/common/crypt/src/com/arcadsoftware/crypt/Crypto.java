...
public final class Crypto {
	...
	public static final int ENCRYPT_ALGORITHM = 3;
	...
	public static String encrypt(byte[] bytes, char[] masterkey, int algorithm) throws NoSuchAlgorithmException, InvalidKeyException, InvalidAlgorithmParameterException, IllegalBlockSizeException, InvalidKeySpecException {
		return encrypt(bytes, masterkey, algorithm, SALTMINSIZE, SALTVARIATION, CIPHERMINITERATIONS, CIPHERITERATIONSVARIATION);
	}
	 ...
	 * The different currently implemented algorithms are :
	 * <ol>
	 * <li> AES-256 with CTR mode, secret key generated by PBKDF2 with Hmac SHA1.
	 * <li> AES-256 with CTR mode, secret key generated by PBKDF2 with Hmac SHA256.
	 * <li> AES-256 with CTR mode, secret key generated by PBKDF2 with Hmac SHA256.
	 * </ol>
	 * 
	 * <p>
	 * All algorithm version use the local limitation of the AES secret key size, but default size is 256.
	 * 
	 * <p>
	 * <strong>Version 1 is not secure.</strong>
	 * 
	 * @param text the clear data to encrypt, the caller is responsible to delete this information after encryption.
	 * @param masterkey The master key in clear text, the caller is responsible to delete this information after encryption.
	 * @param algorithm The algorithm version number to use, currently support only version 1, 2 or 3, version 1 is not secured.
	 * @param saltMinSize define the minimal size of the salt added to the encrypted data.
	 * @param saltVariation is a positive number used to compute the actual size of the salt.
	 * @param minIterations define the minimal number of iterations of the encryption algorithm, a minimal value of 100 is required.
	 * @param iterationsVariation is a positive number used to compute the actual number of iterations of the cipher algorithm.
	 * @return a non null Base64 encrypted buffer.
	 * @throws NoSuchAlgorithmException
	 * @throws InvalidKeyException
	 * @throws InvalidAlgorithmParameterException
	 ...
	 * @throws InvalidKeySpecException
	 * @see #decryptBytes(String, char[])
	 * @see #ENCRYPT_ALGORITHM
	 */
	public static String encrypt(char[] text, char[] masterkey, int algorithm, int saltMinSize, int saltVariation, int minIterations, int iterationsVariation) throws NoSuchAlgorithmException, InvalidKeyException, InvalidAlgorithmParameterException, IllegalBlockSizeException, InvalidKeySpecException {
		byte[] b = getBytes(text);
		try {
			return encrypt(b, masterkey, algorithm, saltMinSize, saltVariation, minIterations, iterationsVariation);
		} finally {
			clear(b);
		}
	}
	 ...
	 * @see #ENCRYPT_ALGORITHM
	 */
	public static String encrypt(byte[] bytes, char[] masterkey, int algorithm, int saltMinSize, int saltVariation, int minIterations, int iterationsVariation) throws NoSuchAlgorithmException, InvalidKeyException, InvalidAlgorithmParameterException, IllegalBlockSizeException, InvalidKeySpecException {
        final byte[] salt = new byte[saltMinSize + SECURERANDOM.nextInt(saltVariation)];
        final int iterations = minIterations + SECURERANDOM.nextInt(iterationsVariation);
        SECURERANDOM.nextBytes(salt);
		...
		try {
			...
			case 1:
				...
				secret = f.generateSecret(new PBEKeySpec(masterkey, salt, iterations, kl));
			...
			try {
		        ...
		        if (algorithm >= 2) {
			        ...
			        setIntByte(result, 12, iterations - minIterations);
		        } else {
			        setIntByte(result, 4, salt.length);
			        setIntByte(result, 8, iv.length);
			        setIntByte(result, 12, iterations);
		        }
	...
	public static char[] decrypt(String text, char[] masterkey) throws NoSuchAlgorithmException, IllegalBlockSizeException, InvalidKeySpecException, InvalidKeyException, InvalidAlgorithmParameterException {
		return decrypt(text, masterkey, SALTMINSIZE, CIPHERMINITERATIONS);
	}
	 ...
	 * @param saltMinSize
	 * @param minIterations
	 * @return null is the original test is null, unFog if fogged, or try to reverse the cypher encryption.
	 * @throws NoSuchAlgorithmException 
	 * @throws IllegalBlockSizeException 
	 * @throws InvalidAlgorithmParameterException 
	 * @throws InvalidKeyException 
	 ...
	 * @see #encrypt(String, char[], int)
	 */
	public static char[] decrypt(String text, char[] masterkey, int saltMinSize, int minIterations) throws NoSuchAlgorithmException, IllegalBlockSizeException, InvalidKeySpecException, InvalidKeyException, InvalidAlgorithmParameterException {
		if (text == null) {
			return null;
		}
		if (text.isEmpty()) {
		...
		byte[] b = decryptImpl(text, masterkey, saltMinSize, minIterations);
		try {
	...
	public static byte[] decryptBytes(String text, char[] masterkey) throws NoSuchAlgorithmException, IllegalBlockSizeException, InvalidKeySpecException, InvalidKeyException, InvalidAlgorithmParameterException {
		return decryptBytes(text, masterkey, SALTMINSIZE, CIPHERMINITERATIONS);
	}	
	/**
	 * Decrypt an encrypted binary data using the given key.
	 * 
	 ...
	 * @see #encrypt(byte[], char[], int)
	 */
	public static byte[] decryptBytes(String text, char[] masterkey, int saltMinSize, int minIterations) throws NoSuchAlgorithmException, IllegalBlockSizeException, InvalidKeySpecException, InvalidKeyException, InvalidAlgorithmParameterException {
		if (text == null) {
		...
		return decryptImpl(text, masterkey, saltMinSize, minIterations);
	}
	
	private static byte[] decryptImpl(String text, char[] masterkey, int saltMinSize, int minIterations) throws NoSuchAlgorithmException, IllegalBlockSizeException, InvalidKeySpecException, InvalidKeyException, InvalidAlgorithmParameterException {
		byte[] iv = null;
		byte[] salt = null;
		byte[] ct = null;
		int iterations = 0;
