...
public class GreaterStrictCriteria extends AbstractSearchCriteria implements Cloneable, IAttributeCriteria {
	...
	public ISearchCriteria reduce(ICriteriaContext context) {
		...
		if (refline.isMultiLinkList()) {
			Activator.getInstance().warn("\"Greater Strict\" Criteria with multi-link references is not supported: " + toString());
			return ConstantCriteria.FALSE;
		}
		if (refline.isLinkList()) {
			...
			if (postLinkCode.isEmpty()) {
				Activator.getInstance().warn("Invalid \"Greater Strict\" Criteria with terminal link reference: " + toString());
			}
			return new LinkGreaterStrictCriteria(preLinkCode, refline.getFirstLinkCode(), postLinkCode, value).reduce(context);
		}
