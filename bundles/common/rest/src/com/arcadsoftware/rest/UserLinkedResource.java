 ...
 * 
 * <p>
 * <b>NB</b>: This code is duplicated with the class {@link UserLinkedIdentifiedResource}.
 * 
 * @see IConnectionUserBean
 */
public abstract class UserLinkedResource extends OSGiResource {
	 ...
	 */
	public static final String QUERY_SUBTITUTETOUSER = "user_subtitude_to"; //$NON-NLS-1$
	 ...
	 * 
	 * @param ref must a valid login or a key code [usertype]/[userid] or [userid].
	...
	protected final boolean hasRight(int key) {
		...
		return (user != null) && user.getProfile().hasRight(key);
	}
	...
	protected final boolean hasRight(int key, int param) {
		IConnectionUserBean user = getUser();
		return (user != null) && user.getProfile().hasRight(key, param);
	}
}
