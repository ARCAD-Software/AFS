...
public class BranchSecure extends SimpleBranch {

	@Override
	protected RouteList createAttachedResources(Context context, Router router) {
		return new RouteList(router.attach("/binredirect/{type}/{id}", new RedirectRestlet(context, Activator.getInstance()))); //$NON-NLS-1$
	}
