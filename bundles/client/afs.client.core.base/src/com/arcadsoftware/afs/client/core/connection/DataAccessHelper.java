...
public class DataAccessHelper {
	...
	public DataAccessHelper(ServerConnection connection) {
		...
		this.dataAccess = connection.getDataAccess();
	}
	...
	public BeanMap getFirst(String type, ISearchCriteria criteria) {
		try {
			BeanMapList l = dataAccess.getList(type, null, criteria, null, 0, -1, false);
			if (l.size()>0) {
				return l.get(0);
			}
		} catch (ServerErrorException e) {
	...
	private String createAttributeParameters(String baseAddress, String attributeList){
		if (attributeList!=null) {
			StringBuilder attributesParameter = new StringBuilder("");
			...
			if (attributes.length>0) {
				...
				if (attributesParameter.length()>0){
					StringBuilder additionalParameters = new StringBuilder("?attributes=");
					//We remove the last '+'
	 ...
	 * @param type The type of the master beanMap
	 * @param id the id of the master bean map
	 * @param associationKey the Association Name 
	 * @param linkedType The type of the returned Bean Map
	 * @return
	 */
	...
	public BeanMapList getLinkList(String type, int id, String associationKey, String linkedType, String attributeList, String orderList, int count){
		try {
			associationKey = createAttributeParameters(associationKey,attributeList);
			BeanMap bean = new BeanMap(type,id);
			return dataAccess.getLinks(bean,associationKey, linkedType,attributeList,null,orderList, 0,count,false);
	...
	public int getCountLinkList(String type, int id, String associationKey, String linkedType){
		BeanMapList list = getLinkList(type, id ,associationKey,linkedType,null);
		if (list == null)
			return 0;
		else
			return list.size();
	 ...
	 * @param associationKey the Association Name 
	 * @param linkedType The type of the returned Bean Map
	 * @param attributeList blank-separated list of attributes
	 * @param orderList blank-separated list of attributes used to order the list
	 * @return
	 */
	public BeanMapList getLinkList(String type, int id, String associationKey, String linkedType, String attributeList, String orderList){
		try {
			associationKey = createAttributeParameters(associationKey,attributeList);
			BeanMap bean = new BeanMap(type,id);
	 ...
	 * @return
	 */
	public BeanMapList getLinkList(String type, int id, String associationKey, String linkedType, String attributeList, String orderList, String criteria){
		try {
			associationKey = createAttributeParameters(associationKey,attributeList);
			BeanMap bean = new BeanMap(type,id);
			return dataAccess.getLinks(bean,associationKey, linkedType,attributeList,criteria,orderList, 0,-1,false);
	...
	/**
	 *  
	 * @param master The master BeanMap 
	 * @param linkCode the link name used
	 * @param subid the id of the bean map to link
	 * @return
	 */
	 ...
	 * @param master Master BeanMap
	 * @param linkCode link code to follow
	 * @param subids list of linked Items ids
	 * @return
	 */
	 ...
	 * @param deleteLinkedItems True if linked items must also be deleted. If False, only links will be removed
	 * @return
	 */
	...
	public void refresh(BeanMap b) {
		try {
			BeanMap res = dataAccess.get(b.getType(), b.getId());	
			if (res != null){
				b.addAll(res);
			}			
		} catch (ServerErrorException e) {
	...
	public void refresh(BeanMap b, String attributeList) {
		try {
			String address = createAttributeParameters("data/"+b.getType()+"/"+b.getId(),attributeList);	
			BeanMap res = dataAccess.get(address, b.getType());	
			if (res != null){
				b.addAll(res);
			}			
		} catch (ServerErrorException e) {
	...
	public BeanMap read(String type, int id, String attributeList) {
		try {
			String address = createAttributeParameters("data/"+type+"/"+id,attributeList);	
	...
	public BeanMap read(String type, ISearchCriteria criteria) {
		BeanMapList l = getList(type, criteria);
		if ((l!=null) && (!l.isEmpty())) {
			return l.get(0);
		}
		return null;
	...
	public InputStream getBeanStream(String type, int id){
		try {
			Representation representation = dataAccess.getBeanStream(type,id);
	...
	public InputStream getBeanStream(BeanMap beanMap){
		try {
			Representation representation = dataAccess.getBeanStream(beanMap.getType(),beanMap.getId());
			if (representation!=null) {
				return representation.getStream();
			}
		} catch (ServerErrorException | IOException e) {
	 ...
	 * @param url web-service able to manage/store/... that file
	 * @param file
	 * @return
	 */
	...
	public String getUploadRedirection(String type, int id){
		try {
			return dataAccess.getUploadRedirection(type, id);
		} catch (ServerErrorException e) {
			manageException(e);
		}			
		return null;
	...
	public boolean put(String url,BeanMap request) {
		try {
			lastMessage=null;
			BeanMap b=dataAccess.put(url,request);
			if (b!=null) {
	...
	public boolean get(String url,BeanMap request) {
		try {
			lastMessage=null;
			BeanMap result = dataAccess.get(url,request.getType());
	...
	public String getRedirection(String path) {
		try {
			return dataAccess.getRedirection(path);
		} catch (ServerErrorException e) {
			manageException(e);
			return null;
		}
