...
public class CertificateDialog extends AbstractAFSDialog {
	...
	protected void okPressed() {
		if (choices[ACCEPT_FROM_SERVER].getSelection()){
			...
			// Open File selector
			String certifFile = GuiFormatTools.choosefile(Activator.resString("server.connection.certificate.accept.title"), CERTIFFILE_FILTERS);
	...
	/**
	 * Get certificate
	 * @param certifFile if null, get certificate from server, else load certificate file
	 * @return
	 */
	private boolean getCertificate(File certifFile) {
		
		String[] hostAndPort = getHostAndPort(server.getUrl());
		...
		InstallCertificates ic = getInstallCertificate(host);
		...
		if (ic.getLastError() > 0 || certificates == null || certificates.size() == 0){
			MessageDialog.openError(getShell(), Activator.resString("server.connection.certificate.error.title.nocertificate"),
					String.format(Activator.resString("server.connection.certificate.error.description.nocertificate"), server.getUrl()));
			Activator.getInstance().log(
					String.format(Activator.resString("server.connection.certificate.error.nocertificate"), host, port), null);
			return false;
		} else {
			StringBuilder message = new StringBuilder();
			for (CertificateInformation certificateInformation : certificates) {
				String sb = Activator.resString("server.connection.certificate.none");

				X509Certificate cert = certificateInformation.getCertificate();
			...
			if (MessageDialog.openQuestion(getShell(), Activator.resString("server.connection.certificate.title.update"),
					String.format(Activator.resString("server.connection.certificate.description"), host, message.toString()))) {
				ic.clear();
				ic.trustCertificates(certificates);
				
				if (ic.getLastError() == InstallCertificates.ERROR_CERTIFICATE_IMPORT){
					MessageDialog.openError(getShell(), Activator.resString("server.connection.certificate.title.storeCertificate"),
							String.format(Activator.resString("server.connection.certificate.error.save"), host, port));
					Activator.getInstance().log(
							String.format(Activator.resString("server.connection.certificate.error.save"), host, port), null);
					return false;
				}
				return true;
			}
