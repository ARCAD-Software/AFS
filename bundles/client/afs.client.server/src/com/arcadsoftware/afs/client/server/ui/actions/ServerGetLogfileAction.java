...
public abstract class ServerGetLogfileAction extends AbstractServerAction {
	...
	private IServerGetLogFileProvider getProvider(){
		...
		for (IConfigurationElement element : elements) {
			try {
				IServerGetLogFileProvider provider = 
							(IServerGetLogFileProvider)element.createExecutableExtension(CLASS_ATTRIBUTE);
				return provider;
	...
	protected boolean execute() {
		...
		IServerGetLogFileProvider provider = getProvider();
		if (provider != null) {
