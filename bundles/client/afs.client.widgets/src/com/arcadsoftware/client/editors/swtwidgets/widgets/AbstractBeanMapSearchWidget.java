...
public abstract class AbstractBeanMapSearchWidget implements IBeanMapContainerValue {
	...
	private final MetaDataFormater attributesFormater;
	protected Text text;
	...
	public AbstractBeanMapSearchWidget(ISWTRenderer renderer, ILayoutParameters parameters, final Element element,
		...
		final String format = parameters.getParameter(FORMAT);
		attributesFormater = (format != null) ? new MetaDataFormater(format, renderer.getStructure(element)) : null;
		final Composite composite = createContainerComposite(horizontalSpan);
	...
	private Composite createDefaultComposite(ISWTRenderer newRenderer, int cols, int horizontalSpan) {
		final Composite composite = newRenderer.getToolkit().createComposite(newRenderer.getParent(), SWT.NONE);
	...
	/**
	 * This method create an ellipsis button that will trigger the search. The search is delegated to an object that
	 * implements ISearchBeanMap interface If the element is readonly the button will be disabled.
	 *
	 * @param composite
	 *            the parent composite
	 * @param element
	 *            the element on which we are working
	 */
	public void createSearchButton(Composite composite, final Element element) {
		...
		searchButton.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent event) {
				final BeanMap newBeanMap = getSelector()
						.search(AbstractBeanMapSearchWidget.this.getRenderer().getStructure(element));
	 ...
	 *            number of horizontal span columns.<br/>
	 *            Typically, this method could be override to change the number of columns provided by the composite
	 *            (default = 2) if you want to add some new content after the search button
	 * @return the parent composite of this widget
	 */
	public Composite createContainerComposite(int horizontalSpan) {
		final Composite composite = createDefaultComposite(renderer, 2, horizontalSpan);
		return composite;
	}
	...
	/**
	 * Override this method if you want to add some other content to the parent composite AFTER the seerach Button.<br/>
	 * Please note that the parent composite creation surely needs to be modified to do that, override the
	 * <code>createContainerComposite</code> method
	 *
	 * @param parent
	...
	/**
	 * Override this method if you want to add some other content to the parent composite BEFORE the search button.<br/>
	 * Please note that the parent composite creation surely needs to be modified to do that, override the
	 * <code>createContainerComposite</code> method
	 *
	 * @param parent
