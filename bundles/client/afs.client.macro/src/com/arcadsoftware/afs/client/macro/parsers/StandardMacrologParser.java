	        	...
	        	if(eventType == XmlPullParser.START_TAG) {
	        		...
	        		if (tag.equals(ITEM_MACRO)) {
	        			if (parser.getAttributeCount()==1) {
	        				String qualifiedName = parser.getAttributeValue(0).trim();
	        				String[] values = qualifiedName.split("/");
	        				if (values.length==2) {
	        		...
	        		if (tag.equals(ITEM_LOG)) {
	        			...
	        			if (parser.getAttributeCount()==count) {
	        				...
	        				String type = parser.getAttributeValue(1).trim();	        				
	        				String status = parser.getAttributeValue(2).trim();
	        				String severity = parser.getAttributeValue(3).trim();
	        				String sequence = parser.getAttributeValue(4);	        				
