...
public abstract class AbstractSearchListComposite extends Composite
	...
	protected AbstractColumnedViewer createTableViewer(Composite parent) {
		...
		final BeanMapListTableViewer viewer = new BeanMapListTableViewer(parent, style, entityStructure,
					...
					provider = new ColumnedDefaultTableLabelProvider(newViewer) {
						...
						protected Image getActualImage(Object element,
							...
							if (actualColumnIndex == 0) {
								...
								final Image image = AbstractSearchListComposite.this.getCustomColumnImage(element,
										actualColumnIndex);
								if (image == null) {
			...
			public SimpleDateFormat getDateFormatter() {
				final SimpleDateFormat sd = AbstractSearchListComposite.this.getDateFormatter();
				if (sd != null) {
					return sd;
				}
			...
			protected String getColumnHeader(String attribute) {
				final String header = AbstractSearchListComposite.this.getColumnHeader(attribute);
				if (header != null) {
					return header;
				}
			...
			protected int getColumnSize(String attribute) {
				final int size = AbstractSearchListComposite.this.getColumnSize(attribute);
				if (size != -1) {
					return size;
				}
			...
			protected Image getCustomColumnImage(Object element,
					int actualColumnIndex) {
				final Image image = AbstractSearchListComposite.this.getCustomColumnImage(element, actualColumnIndex);
				if (image != null) {
					return image;
				}
	...
	protected AbstractColumnedViewer createTreeViewer(Composite parent) {
		final AbstractColumnedViewer myViewer = new BeanMapListTreeViewer(parent,
			...
			protected String getColumnHeader(String attribute) {
				...
				final String newAttribute = AbstractSearchListComposite.this.transformColumnHeader(attribute);
	...
	protected SearchKeyListener createSearchKeyListener() {
		final String attr = getKeySearchAttribute();
		if (attr != null) {
	...
	public void actionDone(int type, BeanMap bean) {
		...
		case IBeanMapActionListener.ACTION_ADD:
			final BeanMap completed = helper.read(bean.getType(), bean.getId(), selectClause);
	...
	/**
	 * This method is called each time a selection event occurs (selection OR seslection) even if the selection is empty
	 * unlike of {@link AbstractSearchListComposite#doOnSelection(BeanMap)} that is triggered only when the selection is
	 * not empty
	 *
	 * @param selection
	 */
