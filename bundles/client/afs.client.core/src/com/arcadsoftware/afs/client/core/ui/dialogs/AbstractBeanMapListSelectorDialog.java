...
public abstract class AbstractBeanMapListSelectorDialog extends AbstractAFSDialog {
	...
	public AbstractBeanMapListSelectorDialog(Shell parentShell,ServerConnection connection,boolean multiselection, boolean withInit) {
	...
	private void setSelection(IStructuredSelection selection) {
		...
		if (!selection.isEmpty()) {
			if (multiselection) {
				Iterator<BeanMap> it = selection.iterator();
				while (it.hasNext()) {
	...
	protected Control createDialogArea(Composite parent) {
		...
		viewer = new BeanMapListTableViewer(composite, style, entity,
				attributeList) {
			@Override
			...
			public AbstractColumnedTableLabelProvider createTableLabelProvider(AbstractColumnedViewer newViewer) {
				AbstractColumnedTableLabelProvider provider = new ColumnedDefaultTableLabelProvider(newViewer) {
					@Override
					...
					public String getColumnText(Object element, int columnIndex) {
						String value = AbstractBeanMapListSelectorDialog.this.getColumnText(element, columnIndex);
						if (value != null)
							return value;
			...
			protected int getColumnSize(String attribute) {
				int size = AbstractBeanMapListSelectorDialog.this.getColumnSize(attribute);
				if (size!=-1)
					return size;
