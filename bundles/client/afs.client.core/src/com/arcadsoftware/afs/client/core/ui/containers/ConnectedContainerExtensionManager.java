...
public class ConnectedContainerExtensionManager {
	...
	private boolean checkRight(ServerConnection connection, String rightId){
		if (rightId!=null) {
			if (StringTools.isNumeric(rightId)) {
				...
				UserMessage um = UserMessageManager.getInstance().getMessage(IACCMessages.ERR_CONTAINER_INVALID_RIGHT, rightId);
		...
		if (parentViewId!=null) { 
			...
			for (IConfigurationElement element : elements) {
				try {
					...
					String rightId = element.getAttribute(RIGHTID_ATTRIBUTE);
					boolean allowed = checkRight(parent.getServerConnection(),rightId);
					if (allowed) {					
						...
						if (everyWhere) {						
							String parentId = element.getAttribute(PARENTID_ATTRIBUTE);
							String containerId = element.getAttribute(CONTAINERID_ATTRIBUTE);
							...
							if (parentId.equalsIgnoreCase(parent.getContainerId())) {								
								...
								if (o instanceof AbstractConnectedContainerProvider) {								
									AbstractConnectedContainerProvider container =  (AbstractConnectedContainerProvider)o;
	...
	public void createChildContainers(AbstractConnectedContainer parent, List<IContainer> mainContainerProviders,String parentViewId) {
		if (parentViewId!=null) { 
			IExtensionRegistry registry = Platform.getExtensionRegistry();
			IConfigurationElement[] elements = registry.getConfigurationElementsFor(EXTENSION_ID);
			for (IConfigurationElement element : elements) {
						...
						if (everyWhere) {						
							...
							if (parentId.equalsIgnoreCase(parent.getContainerId())) {								
								...
								if (o instanceof AbstractConnectedContainerProvider) {								
									...
									AbstractConnectedContainer container =  (AbstractConnectedContainer)o;
									container.setParent(parent);
									container.setContainerId(containerId);
									container.setOrder(getOrder(element));
									mainContainerProviders.add(container);
		...
		if (parentViewId!=null) { 
			...
			for (IConfigurationElement element : elements) {
				try {
					...
					if (allowed) {	
						...
						if (everyWhere) {						
							...
							if (parentId.equalsIgnoreCase(expectedContainerId)) {
								AbstractConnectedContainer container = 
									(AbstractConnectedContainer)element.createExecutableExtension(CLASS_ATTRIBUTE);
								container.setParent(parent);
								container.setContainerId(containerId);
								container.setOrder(getOrder(element));
					...
					UserMessage um = UserMessageManager.getInstance().getMessage(IACCMessages.ERR_CONTAINER_UNKNOWN_ERROR);
					LogUITools.logError(Activator.getDefault().getBundle(),um);					
				}
			}
		...
		return new Comparator<IContainer>(){
			public int compare(IContainer o1,IContainer o2) {
				...
				if (o1 instanceof AbstractConnectedContainerProvider) {
					AbstractConnectedContainerProvider p1 = (AbstractConnectedContainerProvider)o1;
					order1 = p1.getOrder();
				}
				if (o1 instanceof AbstractConnectedContainer) {
					AbstractConnectedContainer p1 = (AbstractConnectedContainer)o1;
					order1 = p1.getOrder();
				}	
				if (o2 instanceof AbstractConnectedContainerProvider) {
					AbstractConnectedContainerProvider p2 = (AbstractConnectedContainerProvider)o2;
					order2 = p2.getOrder();
				}
				if (o2 instanceof AbstractConnectedContainer) {
					AbstractConnectedContainer p2 = (AbstractConnectedContainer)o2;
					order2 = p2.getOrder();
				}								
