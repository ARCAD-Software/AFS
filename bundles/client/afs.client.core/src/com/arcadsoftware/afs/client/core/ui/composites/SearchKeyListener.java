...
public class SearchKeyListener extends KeyAdapter {
	...
	public void keyPressed(KeyEvent e) {	
		if (e.character!=0x0) {
			if (buffer.equalsIgnoreCase(" ")) buffer="";
	...
	public synchronized String createSearchClause(String sc){
		if (buffer.length()>0) {
			...
			StartCriteria equalType = 
					new StartCriteria(attribute,buffer);
			if (sc!=null) {
				String clause =  "<and>"+sc+x.toXML(equalType)+"</and>";
				return clause;
			} else {
				String clause =  x.toXML(equalType);
				return clause;
			}
