...
public class HyperlinkRelatedBeanDecoratorSWTProvider implements IDecoratorSWTProvider {
	...
	public Widget create(ISWTRenderer renderer, ILayoutParameters parameters, MetaDataEntity structure) {
		...
		String attribute = parameters.getParameter(REFERENCE, ""); //$NON-NLS-1$ //$NON-NLS-2$
		String formatString = parameters.getParameter(FORMAT, ""); //$NON-NLS-1$ 
		
		MetaDataAttribute relatedAttribute = structure.getAttribute(attribute);
		String defaultLabel = relatedAttribute.getName();
		...
		if (bean != null) {
			BeanMapFormater formatter = new BeanMapFormater(formatString,bean.getType(),false);
			String text = formatter.format(bean);
	...
	private BeanMap retrieveRelatedBeanMap(ISWTRenderer renderer,
		...
		String relatedType = attribute.getType();
		String relatedCode = attribute.getCode();
		...
		CoreContentLoader loader = (CoreContentLoader)renderer.getDataLoader();
		DataAccessHelper helper =  loader.getHelper(connection);
...
//		}
		...
		BeanMap bean = helper.read(relatedType,current.getInt(relatedCode));
