 ...
 * <p>
 * It displays a selection list based on a BeanMapList that the user has to fill and passed to this
 * page using the {@link #getInput()} method or by directly calling the {@link #setInput(BeanMapList)} one.
 * <p>
 * You can override the method {@link #createAreaAfter(Composite)} or {@link #createAreaBefore(Composite)}
 * methods to add some graphical element after or before the selection list.
 * </p>
 * <p>
 * Get the user selection using the {@link #getSelectedItem()} or {@link #getSelectedItems()} method.
 * Note that the {@link #getSelectedItem()} return the first element of the selection if the multiselection
 * mode has been activated using the {@link #isMultiSelectionAvailable()} overrode method (true by default)
 * </p>
		...
		list = new BeanMapListAdapter(innerComposite,getType(),connection){
			...
			protected SimpleDateFormat getDateFormatter() {
				SimpleDateFormat sdf = 
						AbstractConnectedSimpleSelectionWizardPage.this.getDateFormatter();
				if (sdf==null) {
			...
			protected String getDisplayedSelectClause() {
				String result = AbstractConnectedSimpleSelectionWizardPage.this.getDisplayedSelectClause();
				if (result==null) {
