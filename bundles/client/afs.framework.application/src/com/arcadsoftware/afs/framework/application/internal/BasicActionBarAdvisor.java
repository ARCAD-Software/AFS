...
public class BasicActionBarAdvisor extends ActionBarAdvisor implements IActionRegister {
	...
	/**
	 * This operation allows cleaning action Sets coming form eclipse plugins, that can not be cleaned another way.
	 * Not really clean... But Efficient!
	 */
	...
	private void createFileMenu(IMenuManager menuBar){
		if (isMenuVisible( IMENUConst.FILE)){
			MenuManager fileMenu = 
					new MenuManager(getMenuLabel(IMENUConst.FILE), IWorkbenchActionConstants.M_FILE);
	...
	private void createEditMenu(IMenuManager menuBar){
		if (isMenuVisible( IMENUConst.EDIT)){
			MenuManager editMenu = 
				new MenuManager(getMenuLabel(IMENUConst.EDIT), IWorkbenchActionConstants.M_EDIT);
	...
	private void createHelpMenu(IMenuManager menuBar){
		if (isMenuVisible( IMENUConst.HELP)){
			MenuManager helpMenu = 
				new MenuManager(getMenuLabel(IMENUConst.HELP), IWorkbenchActionConstants.M_HELP);
	...
	private void createWindowMenu(IMenuManager menuBar){
		if (isMenuVisible( IMENUConst.WINDOW)){
			MenuManager helpMenu = 
				new MenuManager(getMenuLabel(IMENUConst.WINDOW), IWorkbenchActionConstants.M_WINDOW);
			menuBar.add(helpMenu);	
	...
	private boolean loadMenuFactory() {
		for (IConfigurationElement element : Platform.getExtensionRegistry().getConfigurationElementsFor(IBasicMenuFactory.MENUFACTORY_EXTENSION)) {
			try {
				menuFactory = (IBasicMenuFactory) element.createExecutableExtension("class");
				return true;
			} catch (CoreException e) {
				Activator.getInstance().getLog().log(new Status(Status.ERROR, // 
	...
	private void createStandardMenus(IMenuManager menuBar){
		if (loadMenuFactory()) {
			...
			Activator.getInstance().getLog().log(new Status(Status.WARNING, // 
					Activator.getInstance().getBundle().getSymbolicName(), //
