...
public class ContainerExtensionManager {
	...
	private ContainerReference createContainer(IConfigurationElement element) {
		final ContainerReference childContainer = new ContainerReference();
		...
		final String iconKey = element.getAttribute("icon");//$NON-NLS-1$
		if (iconKey != null) {
	...
	public void createExtensions(List<ContainerReference> list) {
		...
		for (final IConfigurationElement element : elements) {
			final ContainerReference childContainer = createContainer(element);
	...
	public void createChildExtensions(List<ContainerReference> list, String parentId) {
		for (final IConfigurationElement element : Platform.getExtensionRegistry()
				.getConfigurationElementsFor(EXTENSION_ID)) {
			final String category = element.getAttribute("category");//$NON-NLS-1$
