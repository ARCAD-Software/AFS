...
public class CLI extends LoggedObject{
	...
	public static void main(String[] args) {	
		...
		m.initProperties();
		ExecutionResult result = m.execute(args);
	...
	private boolean loadFromFolder() throws Throwable{
		...
		File serviceFolder = new File("services");
		...
		if (serviceFolder.exists()) {
			...
			if (files.length>0) {
				...
				log("No service File found in "+serviceFolder.getAbsolutePath()+" directory",IMessageLogger.LOGLVL_FATAL);
				return false;
			}
		} else {
			log("File Service Directory "+serviceFolder.getAbsolutePath()+" not found",IMessageLogger.LOGLVL_FATAL);
		}
		return false;
	}
	
	public ExecutionResult execute(String[] args) {
		ISimpleLogger logger = createLogger();
		this.setLogger(logger);
		ServiceFactory.getInstance().setLogger(logger);
		
		AbstractService service;
		try {			
			File f = new File(getServiceFilename());
			if (!f.exists()) {
				log("Service File Name: "+getServiceFilename()+" not found",IMessageLogger.LOGLVL_WARNING);
				boolean searchInFolder = loadFromFolder();
				if (! searchInFolder) {
			...
			if (service!=null){
				...
				ExecutionResult result = executeService(args, service);
