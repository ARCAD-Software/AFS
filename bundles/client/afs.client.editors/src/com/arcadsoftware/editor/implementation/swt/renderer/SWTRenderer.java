			...
			if (internalEditors.canSavedInternalEditors() && canSavedEditor()) {
				if (internalEditors.saveInternalEditors()) {
					...
					if (saved) {
						...
						fireSaveEvent();
						// FIXME SÃ©mantiquement le BeanMap n'a pas changÃ© pourquoi dÃ©clencher un Ã©vÃ¨nement Changed
						// ?
						// FIXME Surtout que cet Ã©vÃ¨nement vient dÃ©jÃ  d'Ãªtre dÃ©clanchÃ© 10 lignes plus haut
						// (reload()).
						fireChangedEvent();
					}
					result = saved;
