...
/**
 * Most of this implementation is from Robert Harder's public domain Base64 code
 * (version 1.4.1 available from &lt;http://iharder.net/xmlizable>).
 * 
...
public class Base64 {
	...
	private static final String NAME = "Base64"; //$NON-NLS-1$
	private static final boolean DEBUG = true;
	private static final int debuglevel = 9;
	private static final PrintWriter err = new PrintWriter(System.out, true);

	private static void debug(String s) {
		err.println(">>> " + NAME + ": " + s); //$NON-NLS-1$ //$NON-NLS-2$
	}
	...
	public static final byte[] decode(final String s) throws UnsupportedEncodingException {
		final byte[] bytes;
		bytes = s.getBytes("US-ASCII"); //$NON-NLS-1$
	...
	public static byte[] decode(final byte[] src, final int off, final int len) {
		...
		for (i = off; i < off + len; i++) {
			...
			if (sbiDecode >= WHITE_SPACE_ENC) { // White space, Equals sign or
				...
				throw new IllegalArgumentException("Base64.IllegalCharacter" + i + ": " + src[i] + "Base64.Decimal"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
			}
		}
	...
	private static final int decode4to3(final byte[] src, final int sOffset, final byte[] dest, final int dOffset) {
		...
		try { // Example: DkLE
			...
			if (DEBUG && debuglevel > 8) {
				debug("" + src[sOffset] + ": " + (DECODABET[src[sOffset]])); //$NON-NLS-1$ //$NON-NLS-2$
				debug("" + src[sOffset + 1] + ": " + (DECODABET[src[sOffset + 1]])); //$NON-NLS-1$ //$NON-NLS-2$
				debug("" + src[sOffset + 2] + ": " + (DECODABET[src[sOffset + 2]])); //$NON-NLS-1$ //$NON-NLS-2$
				debug("" + src[sOffset + 3] + ": " + (DECODABET[src[sOffset + 3]])); //$NON-NLS-1$ //$NON-NLS-2$
			}
			return -1;
		}
	}
